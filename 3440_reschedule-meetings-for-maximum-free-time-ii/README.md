...
- Objective:
	- Reschedule **1** meeting to a different start time with same duration
	- So that consecutive free time is maximized
	- Return such maximum free time
- Intuition:
	- For `n` events, there are `n+1` gap of free time
	- There are 2 way to reschedule a event `e`:
		1. swap `e` with an adjacent gap : 
			- `1[1]1[2]1 -> 11[1][2]1`
			- total free time = gap left to `e` + gap right to `e`
		2. swap `e` with a non-adjacent gap, if `gap.len >= e.len` : 
			- `1([1])1[2](1) -> 111[2][1]`
			- total free time = gap left to `e` + gap right to `e` + `e.len`
	- Method (2) will yeild longer free time, hence more preferable
- Implementation:
	- Construct array of free time `gaps` -- $O(N)$
	- For each event: -- $O(N)$
		- test for swap (1)
		- test for swap (2): -- $O(N+1)$
			- test for `g` in `gaps`
- Optimization:
	- Sorting a copy of `gaps` by length while keeping index and exit loop early.
		- `gaps_sort = [(idx,len)]` then `gaps_sort.sort(reverse=True, key=lambda x:x[1])`
		- test for swapping with non-adj gap, `break` when `gaps_sort[j] < endTime[i] - startTime[i]`
