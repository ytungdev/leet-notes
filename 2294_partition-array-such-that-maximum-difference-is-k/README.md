...
- Intuition:
	- `sort()` `nums` to minimize difference between adjacent element
	- Iterate `nums[i]` and split remaining array if `nums[i]` is out of range for current subarray
- Implmentation:
	- Track first element of current subarray as `curr`
	- Track total number of subarray as `ret`
	- For each `nums[i]`:
		- If `curr + k < nums[i]` : add partition, `curr=nums[i]` and `ret+=1`
	- return `ret`
