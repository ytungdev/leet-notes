...
- Idea:
	- use hashmap (dictionary) to dynamically keep track of frequency of numbers in `nums`
	- for each index `i`:
		- check if `dominant` criteria met in left-subarray, if not check next index `i`
		- check if `dominant` criteria met in right-subarray, if so return index `i`

- Implementation:
	- use `ferq_L` and `freq_R` to keep track of frequency of integer in `nums[0:i+1]` and `nums[i+1:]`
	- iterate through `nums` and update `freq_R` for each element
	- find `dominant` with highest frequency in `freq_R`
	- iterate through `i -> len(nums)-1`, for each index `i` (`n`=`nums[i]`):
		- `freq_L[n]` += 1
		- `freq_R[n]` -= 1
		- if `dominant` criteria not met in left array : continue to next iteration
			- `freq_L[dominant] * 2` <= i + 1: continue
		- else (`dominant` criteria met in left subarray):
			- if `dominant` criteria met in rgith subarray : return index `i`
	- return -1 if no index met question requirement

- Enhancement:
	- Only keep track of `dominant.freq` in both subarray
