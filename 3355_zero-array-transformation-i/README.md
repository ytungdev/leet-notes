...
- Intuition:
	- Let `steps[i]` be the total step after all queries for `nums[i]`.
	- Use difference array `d[i]` to proccess all queries in $O(Q)$ time.
	- Use prefix sum of `d[i]` to calculate `steps[i]` for each `i` in $O(N)$ time.
	- Compare `steps[i]` and `nums[i]` for each `i`, return `False` if any `nums[i] - steps[i] > 0`.
