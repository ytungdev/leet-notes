...
- time  : $O(N \cdot logN + \frac{N}{3})$
	- `sort()` take $O(N \cdot logN)$
	- Constructing result array take $O(\frac{N}{3})$
- space : $O(N)$
	- `sort()` in python take $O(N)$

---

- Intuition:
	- Sort `nums` to minimize difference between adjaccent element.
	- Create result array `res` with `n//3` element
	- For each index in `nums` that is multiple of 3:
		- If `nums[i]+k < nums[i+2]`, return `[]`
		- Else, `res[i//3]  = [nums[i],nums[i+1],nums[i+2]]`