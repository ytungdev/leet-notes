...
- time  : $O(N^2L)$, where
	...
	- `L` is the length of longest word in `words`
- space : $O(N^2)$, worst case storing full path in nested array

---

- Intuition:
	- For adjacent element `i` and `j`, `fulfilling(i,j)` is `True` if:
		- `groups[i] != groups[j]` and 
		- `hamming(i,j)==1` -- $O(L)$
	- Denote longest fulfilling subsequence ending at index `i` as `dp[i]`
    	- `dp[j]=max(dp[j],dp[i]+1)` if `fulfilling(i,j)` for `i<j`
	- Store table of subsequqnce `dp[i]` as :
		- `dp_l[i]`: length of subsequence, default to 1
		- `dp_a[i]`: array of subsequqnce, default to `words[i]`
	- Default global max length and corresponding ending index `max_l=1` and `max_i=0`
	- For each index `j` for `[0..n)`, -- $O(N)$
		- Let local max length `best_l` = `dp_l[j]`, which is 1 by default.
		- Find previous `fulfilling(i,j)` -- $O(N)$
		- Update `best_l` if `dp_l[i]+1 > best_l`
		- Check and update `max_l` for each fulfilling `i,j`
	
- Implementation:
	- Build `dp` table by bottom-up DP. For each index `R` for `[1..n)`:
		- Find the previous index that fulfill `fulfilling(L,R)` by iterate `L` for `[n-1..0]`
		- Update `dp_a[R]=dp_a[L]+words[R]` and `dp_l[R]=dp_l[L]+1`

- Alternative:
	- Track `dp_l` and `prev` instead of `dp_l` and `dp_a`, then build result array with `prev`
	- Trade off performance for memory usage -- $O(N^2) \rightarrow O(N)$