...
- Intuition:
	- Keep track of minimum value up to `i` in `[0..n)` as `min_v`
	- Keep track of maximum diff between `nums[i]` and `min_v` as `max_diff`
	- Update `min_v` and `max_diff` while iterating `i`
- Implementation:
	- For each `i` in `[1..n)`, compare `nums[i]` and `min_v`:
		- If `nums[i]` > `min_v`:
			- Update `max_diff` if `nums[i] - min_v` > `max_diff`
		- Else:
			- Update `min_v = nums[i]`
	- Return `max_diff`
