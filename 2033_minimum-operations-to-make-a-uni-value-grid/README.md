...
- flatten grid into array `arr`
- sort `arr`
- find median `median` of array (ensure 50/50 of number is higher/lower than the median)
- for `n` in `arr`:
	- calculate `step` by `abs(n-median)/x`
	- if `step` is interger:
		- `result` += `step`
	- else:
		- return `-1`
...
- Enhancement:
	...
	- return ealier if any of the number cannot be reached by x-sized step
