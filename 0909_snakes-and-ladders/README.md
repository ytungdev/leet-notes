...
- Intuition:
	- Construct linear array `seq` for easier computation
	- Denote `dp[i]` as min roll to reach cell `i`
		- Initial value of all cell is `inf`
		- set `dp[1]=0`
	- Setup for BFS:
		- `visited = [False]*n`
		- `q = deque([1])`
	- While `q` is not empty, BFS for next 6 cell:
		- If `visited[curr]`: continue
		- Set `visited[curr] = True`
		- For `d` in `range(1,7)`:
			- If no snake or ladder: 
				- `dp[curr+d] = min(dp[curr+d], dp[curr]+1)`
				- Add `curr+d` to `q` if not visited
			- Else : 
				- `dp[dest[curr+d]] = min(dp[dest[curr+d]], dp[curr]+1)`
				- Add `dest[curr+d]` to `q` if not visited
     - return -1 if `dp[-1]==inf`