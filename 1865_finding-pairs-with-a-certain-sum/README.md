...
- time  : 
	- `init()`: $O(N_1+N_2)
	- `add()` : $O(1)$
	- `count()` : $O(S_1)$, where $S_1$ is the number of distinct element in `nums1`
- space : $O(N)$

---

- Intuition:
	- `count()` can be done by:
		- (1) Iterate `num` in `set(nums1)` and lookup frequency of `tot-num` in `nums2`, or
		- (2) Iterate `num` in `set(nums2)` and lookup frequency of `tot-num` in `nums1`
		- `ret += nums1.count(num) * nums2.count(tot-num)`
		- `Counter()` can be used.
	- `add()` need to update `counter()` of `nums2` as well
- Optimization:
	- `nums1` is constant through out the process and have a smaller size
	- During iteration of `num` in `nums1` for `count()`,
		- If `num > tot`, it is not possible to find `i,j` pair
		- All element `num_i` in `nums1` with `num_i >= num` can also be skipped
	- We can pre-process keys in `nums1` by sorting
	- End iteration when `num > tot`