...
- time  : $O(N+M)$
	- Where `N` is the lenght of `s1` and `s2`, `M` is the length of `baseStr`
	- Worst-case for `find()` is $O(26)$
	- Building DSU(`union()`) take `N` time `find()`
	- Building result string `res` take `M` time `find()`
- space : $O(1)$
	- `uf` have a fixed length of 26

---

- Intuition:
	- Create `dsu` to store root of each character (a-z)
	- Define `find(char)` to return root of given `char` in `dsu`
	- Build disjoint set `dsu` with `s1` and `s2`
	- Build result string `res` by replacing each `char` in `baseStr` with `find(char)`
- Optimization:
	- Path compression is not needed, as max depth of tree is 26, which is small
	- Use `dict` instead of `list` to represent `dsu` to skip char-digit conversion