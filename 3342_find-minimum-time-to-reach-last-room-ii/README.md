...
- time  : $O(nm \times log(nm))$
- space : $O(nm)$

---

- Technique: Dijkstra's algorithm
- Library: `python.heapq`
- Intuition:
	- Each `heappush/heappop` take $O(log(nm))$, at most $nm$ point to insert/delete
	- Use `heapq.heappush(heap, (time,row,col,step))` to represent state in heap
		- `step` = `2` if `last_step` == `1` else `1`
	- For entering `room[i][j]` from `room[r][c]`, 
		- mimimum time after entering the room is `max(time[r][c], moveTime[i][j]) + step`