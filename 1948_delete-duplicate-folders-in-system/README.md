...
- time  : $O(NL \cdot W log W)$, 
	- `W` is the max width of the tree
	- `L` is the max length of paths
	- Building trie take $O(L \cdot N)$
	- Serializing with sort take $O(NL \cdot W log W)$
	- Marking valid path take $O(NL)$
- space : $O(NL)$

---

- Objective:
	- Detect repeating structure of sub tree of `folder`
	- Return path without repeating sub tree
- Intuition:
	- Construct trie to reduce iteration
	- Serialize children of each node and count freq of each pattern
		- Sort serial of children to compare child with different order
		- `node.serial = "folder1(folder11(folder111()))folder2()"`
	- Mark valid path
- Implementation:
	- Serialize : post-order dfs
	- Marking : pre-oder dfs