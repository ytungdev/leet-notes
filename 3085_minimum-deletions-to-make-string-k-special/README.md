...
## Solution - Enumerate character set with nested loop

- time  : $O(N + C^2)$, where `C` is the length of character set in `s`
- space : $O(C)$

---

- Intuition:
	- To achive K-special, frequency of certain character `fix_char` remain unchage.
	- There are two way to delete character `char` from `s` to achieve K-special:
		- Delete all occurence of `char` if `freq[char] < freq[fix_char]`
		- Delete extra `char` if `freq[char] > freq[fix_char] + k`
	- Enumerate all possible `fix_char`
	- Return minimum deleteion.