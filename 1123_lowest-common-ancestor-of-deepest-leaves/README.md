...
- Use post-order DFS returning `(height,LCA_subtree)` for given `node`
- for each node:
	- base case : if `node` is `None`, return `(0,None)`
	- evaluate height and LCA subtree of left subtree by recursion
		- `(subtree_L_height, subtree_L)`
	- evaluate height and LCA subtree of right subtree by recursion
		- `(subtree_R_height, subtree_R)`
	- if `subtree_L.height` == `subtree_R.height` : current node is LCA
		- return `(subtree.height, node)` to previous recursion call
	- else: current node is not LCA
		- return highest subtree `(subtree_L/R_height + 1, subtree_L/R)`
