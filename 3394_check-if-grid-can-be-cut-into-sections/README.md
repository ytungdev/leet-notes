...
- iterate throguh `rectangles` to get array of intervals in both x and y direction
	- for `retangle` in `rectangles` **O(N)**
		- `x1,y1,x2,y2` = rectangles
		- `x_ranges`.append(`[x1,x2]`)
		- `y_ranges`.append(`[y1,y2]`)
- sort both array `x_ranges` and `y_ranges` by the first element in subarray **O(NlogN)**
	- `sorted(i_ranges, key=lambda x:x[0])`
- merge overlapping range in both array **O(N)**
	- for `interval` in `intervals`
		- check if `interval.start` >= `last_interval.end`
			- no overlap
			- add `last_interval` into `result`
		- else:
			- overlaped
			- update `last_interval.end` = max(`interval.end`, `last_interval.end`)
	- add `last_interval` into result
- if x or y direction have 3+ resulting intervals, return `True`
