...
- Intuition:
	- Rescheduling a event means swaping a event with a free time
	- Rescheduling `k` event means merging `k+1` adjacent free time
- Implementation:
	- Store all free time between events, including free time with `0` duration
	- Use sliding window to find maximum sum of consecutive free time with window length `k+1`