...
- Intuition:
	- Keep track of last non-equal element `prev`
		- Only update when `nums[i] != nums[i+1]`
	- Only do comparison when value of element is going to change
		- Compare `prev, nums[i], nums[i+1]` when `nums[i] != nums[i+1]`
	- First and last element will not be hill or valley
		- Iterate from `[1,n-1)`

#### Approach 2 - 2-pass iteration (reduce to set and find turning point)
...
- Intuition:
	- Create `num_set` by iterate `nums` and skip repeating element
	- If `len(num_set)` < 3: there is no hill or valley
	- We can count turning point by tracking direction(up/down) and previous value
- Implementation:
	- Find initial direction with `num_set[0]` and `num_set[1]`
	- For each `nums[i]`:
		- `ret += 1` if 
			- `nums[i] > nums[i+1]` when direction is `up`, or
			- `nums[i] < nums[i+1]` when direction is `down`
			- `nums[i]*dir > nums[i+1]*dir` where `dir=-1` for `down` and `dir=1` for `up`