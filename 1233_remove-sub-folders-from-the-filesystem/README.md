...
- time  : $O(N log N \cdot L)$, where `L` is the max lenght of path
- space : $O(N \codt L)$

---

- Objective:
	- Only add path without repeating subfolder
- Intuition:
	- Sort `folder` in lexicographically, so that 
		- adjacent `folder` start with same path
		- among `folder` with same path, shortest path always come first
- Implementation:
	- Iterate `folder`:
		- if `curr.startswith(prev)` : skip
		- else `ret.append(curr)` and `prev = curr`