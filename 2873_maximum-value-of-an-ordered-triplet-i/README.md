...
- iterate with 3 nested loop to check every possibility of triplets
- update `res` if calculated result with `i,j,k` is greater than `res`
- return `res`
...
- for each iteration compare and update `max_i`, `max_diff` and `max_value`
- return `max_value`