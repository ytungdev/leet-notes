...
- sort meetings with starting day `sorted(meetings, key=lambda x:x[0])`
- set `last_meeting` as `[0,0]` and `available_day` as `0`
- for each `meeting` check : 
	- if `meeting` overlap with `last_meeting`:
		- `last_meeting.start` = `min(last_meeting.start, meeting.start)`
		- `last_meeting.end`   = `max(last_meeting.end, meeting.end)`
	- else (no overlap):
		- `available_day` += `meeting.start` - `last_meeting.end` - 1 (only available if meeting seperate by at least 2 day)
