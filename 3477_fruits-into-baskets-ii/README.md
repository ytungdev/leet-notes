...
- Objective:
	- Find left-most available baskets for each fruits
	- Return number or unplaced fruits
- Intuition:
	- To mark `basket[i]` as taken, we can either:
		- Remove `baskets[i]` from the list; or
		- Modify the value of `baskets[i]` to `0`, so that other fruit cannot be placed
	- We can return any of the following value depending on implementation:
		- Length of remaining baskets (if deleting element)
		- `n - t`, when `t` is the number of taken baskets
		- Number of untaken baskets `u`
