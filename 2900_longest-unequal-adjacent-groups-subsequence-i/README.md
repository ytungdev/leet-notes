...
- time  : $O(N)$, where `N` is the length of `words` or `groups`
- space : $O(A)$, where `A` is the length of answer array
...
- Intuition:
    - Longest subsequence with alternating group can be found by:
        ...
        - (3) Take the element
        - Repeat (2) and (3) until end of array `words`
- Implementation:
    - Let target group `grp` to be the group of first element(`groups[0]`)
    - For `i` in range `[0->n)`
        - If group of current element(`groups[i]`) equal `grp`: add to `res`, alternate `grp^=1`
